...
async def _write_diary_background(context, transcript_path, cwd):
    ...
    try:
        ...
        prompt = (
            "Посмотри что было сегодня (git log, файлы которые менялись, задачи). "
            "Если ничего значимого не произошло, НЕ создавай файл, просто ответь что записывать нечего. "
            "Если были значимые задачи, запиши дневник в " + diary_file + ". "
